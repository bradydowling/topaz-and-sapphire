...
class Warrior_Twitter extends WP_Widget {
	...
	function Warrior_Twitter() {
		$widget_ops  = array( 'classname' => 'latest-tweets', 'description' => __('Warrior Twitter Tweets Widget', 'infashion') );
		$control_ops = array( 'id_base' => 'warrior_twitter' );

		$this->WP_Widget( 'warrior_twitter', __('Warrior Twitter Tweets', 'infashion'), $widget_ops, $control_ops );
	}
...
?>
		...
		<?php echo $before_title . $warrior_twitter_title . $after_title; ?>
			...
			<?php if ( $tweets ) : ?>
				<?php foreach ( $tweets as $tweet ) : ?>
					<li>
						...
						<div class="entry-meta"><span><i class="fa fa-clock-o"></i> <?php echo date('F jS, Y', strtotime($tweet['created_at']) ); ?></span></div>
					</li>
				<?php endforeach; ?>
			<?php endif; ?>
	        </ul>
			...
			<div class="read-more">
        		<a class="button" target="_blank" href="http://twitter.com/<?php echo $warrior_twitter_username;?>"><?php echo $warrior_twitter_button_text; ?></a>
        	</div>
		</div>
<?php
	echo $after_widget;
	...
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array('warrior_twitter_title' => __('Latest Tweets', 'infashion'), 'warrior_twitter_username' => 'themewarrior', 'warrior_twitter_tweets_count' => '1', 'warrior_twitter_button_text' => __('Follow me on Twitter', 'infashion'), 'twitter_consumer_key' => '', 'twitter_consumer_secret' => '' ) );
	?>
        <p>
            <label for="<?php echo $this->get_field_id( 'warrior_twitter_title' ); ?>"><?php _e('Widget Title:', 'infashion'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'warrior_twitter_username' ); ?>"><?php _e('Twitter Username:', 'infashion'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'warrior_twitter_tweets_count' ); ?>"><?php _e('Tweets Count:', 'infashion'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'warrior_twitter_button_text' ); ?>"><?php _e('Button Text:', 'infashion'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_consumer_key' ); ?>"><?php _e('Twitter Consumer key:', 'infashion'); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'twitter_consumer_secret' ); ?>"><?php _e('Twitter Consumer secret:', 'infashion'); ?></label>
		...
		<p><?php _e('You can get Twitter consumer key & consumer secret by <a href="http://dev.twitter.com" target="_blank">creating an app</a> on Twitter.', 'infashion'); ?></p>
	<?php
	}
}
...
function warrior_get_recent_tweets( $screen_name = '', $consumer_key = '', $consumer_secret = '', $tweets_count = 3 ) {
	...
	if (false === $recent_tweets) {
		...
		if (!is_wp_error($response)) {
			...
			echo '<p>'. __('Error: Can\'t fetch tweets, your Twitter account probably protected. Please unprotect your account.', 'infashion') .'</p>';
		}
