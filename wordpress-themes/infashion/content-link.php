...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'link post-detail blog-post hentry' ); ?>>
	<header class="entry-header">
		<div class="post-icon"><i class="icon icon-link"></i></div>
		<?php if( ! is_single() ) : ?>
			<?php if( function_exists( 'get_field' ) && get_field( 'post_format_link' ) ) : ?>
					<h3 class="post-title"><a href="<?php echo esc_url( get_field( 'post_format_link' ) ); ?>" target="_blank"><?php the_title(); ?></a></h3>
			<?php else: ?>
				<h3 class="post-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
			<?php endif; ?>
		<?php else: ?>
			<h1 class="post-title"><a href="<?php echo esc_url( get_field( 'post_format_link' ) ); ?>" target="_blank"><?php the_title(); ?></a></h1>
		<?php endif; ?>	
	</header>
